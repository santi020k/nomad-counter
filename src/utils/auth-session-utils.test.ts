...
describe('auth-session-utils', () => {
  ...
  it('handleGoogleSignIn calls toastError on error', async () => {
    const signInWithOAuthSpy = vi.spyOn(supabase.auth, 'signInWithOAuth')
    const toastErrorSpy = vi.spyOn(toasts, 'toastError')
    const error = new Error() as AuthError
    const data = { provider: 'google' as Provider, url: null }
    signInWithOAuthSpy.mockResolvedValue({ data, error })
    await handleGoogleSignIn()
